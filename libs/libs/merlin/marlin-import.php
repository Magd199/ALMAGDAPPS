...
function merlin_local_import_files() {
	return array(
		array(
			...
			'local_import_file'            => get_parent_theme_file_path( '/libs/demo/demo-content.xml' ),
			'local_import_widget_file'     => get_parent_theme_file_path( '/libs/demo/demo-widgets.wie' ), 
			'local_import_customizer_file' => get_parent_theme_file_path( '/libs/demo/demo-data.dat' ), 
			'import_preview_image_url'     => get_template_directory( '/screenshot.png' ), 
			'local_import_redux'           => array(
				array(
					'file_path'   => trailingslashit( get_parent_theme_file_path() ) . '/libs/demo/demo-redux.json',
					'option_name' => 'opt_themes',
				),
			),
			'import_notice'              => __( 'Before Setup Demo '.EX_THEMES_NAMES_.' Please Install All Plugin Requireds. For Import You Need to wait 3-5 Mintues.', 'exthemes' ),
