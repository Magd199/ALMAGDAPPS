...
function wp_apkstore() {
$sources = 'apk-store.org';
$demos = 'https://apk-store.org/shadow-of-death-2-mod-apk.html'; ?>
    <div class="play_menu" style="text-transform:uppercase">
        <b>Add sources from apk-store.org</b>
    ...
    if(isset($_POST['wp_sb'])) {
        if(isset($_POST['wp_url'])) {
            ...
            if(stristr($url, 'apk-store.org')) {
                require_once 'apkstore.class.php';
    ...
    <div class="wrap">
        <div class="play_fixedx play_menu" id="play_fixed" style="margin-bottom: 10px;">
            ...
            <ul class="play_menus" style="text-transform:capitalize">
                ...
                <li><a data-toggle="tab" href="#menu5"><i class="fa fa-rss"></i> Latest Mods</a></li>
        ...
        <div class="tab-content">
			...
			<div id="menu5" class="tab-pane fade">
               ...
               <?php
                    ...
                    $sumbers = 'https://apk-store.org/mod'; 
                    $target1 = 'https://apk-store.org';
                    $target2 = 'apk-store.org';
					$contents =  file_get_contents($sumbers, false, stream_context_create($ssl)); 
					
					$contents =  preg_replace('#<!DOCTYPE html>.*?<body>#si', '', $contents);   
					$contents =  preg_replace('#<header.*?</header>#si', '', $contents);   
					$contents =  str_ireplace('data-src="', 'src="', $contents); 
					$contents =  preg_replace('#<main.*?<div class="short_block">#si', '', $contents);   
					$contents =  preg_replace('#<div class="navigation">.*?</html>#si', '', $contents);   
					$contents =  preg_replace('#<div class="s-rate-app">.*?</div>#si', '<div class="s-rate-app"></div>', $contents);   
					$contents =  preg_replace('#<div class="short_block_link_info">.*?</a>#si', '</a>'.PHP_EOL.PHP_EOL, $contents);  
					
					$contents =  preg_replace('#<a href="(.*?)" class="short_block_link">.*?<img src="(.*?)".*?class="short_block_link_image">.*?<div class="short_block_link_title">(.*?)</div>.*?</a>#si', '
					<div class="containerx1">
					<div class="item item--1"><img src="$2" width="75px" height="75px" /></div>
					<div class="item item--2"><b>$3</b></div>
					<div class="item item--3"><form name="myForm" id="myForm" method="POST" class="sidebar" style=""  ><input style="display:none" type="text" name="wp_url" value="$1"  /><input type="submit" id="Submit" name="wp_sb" class="button-primary abe2" value="'.postnow2.'"/></form></div> 
					</div>', $contents);
					echo $contents;
					?> 
            ...
            <div id="menu2" class="tab-pane fade">
				...
				<b>Latest Uploads Games </b>
                    ...
                    $sumbers = 'https://apk-store.org/games'; 
                    $target1 = 'https://apk-store.org';
                    $target2 = 'apk-store.org';
					$contents =  file_get_contents($sumbers, false, stream_context_create($ssl)); 
            ...
            <div id="menu3" class="tab-pane fade">
			   <div class="play_menu" style="text-transform:uppercase">
				...
				<?php
                    ...
                    $host2 = $host;
                    $host3 = $host;
                    $sumbers = 'https://apk-store.org/apps'; 
                    $target1 = 'https://apk-store.org';
                    $target2 = 'apk-store.org';
					$contents =  file_get_contents($sumbers, false, stream_context_create($ssl)); 
