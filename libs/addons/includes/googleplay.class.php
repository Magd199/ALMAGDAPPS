...
{	
	...
	{				
		...
		$arr['title_id'] = $this->match('/<link rel="canonical" href=".*?\?id=(.*?)\&.*?">/ms', $links, 1);
		$arr['GP_ID'] = $this->match('/<link rel="canonical" href=".*?\?id=(.*?)\&.*?">/ms', $links, 1);
		$title_id = $this->match('/<link rel="canonical" href=".*?\?id=(.*?)\&.*?">/ms', $links, 1);		
...
private function get_web_id_from_search($title, $engine = "yahoo"){
		...
		$url = "http://www.${engine}.com/search?q=play.google.com+" . rawurlencode($title);
		/*
