...
class getslinks {	
	...
	{				
		...
		$arr['title_id'] = $this->match('/<\/svg>.*?Google Playstore.*?<\/th>.*?<td.*?<a href=".*?\?id=(.*?)\.*?".*?<\/a>/ms', $links, 1);	
		
		$arr['GP_ID'] = $this->match('/<\/svg>.*?Google Playstore.*?<\/th>.*?<td.*?<a href=".*?\?id=(.*?)\.*?".*?<\/a>/ms', $links, 1); 
		
		$arr['GP_IDX'] = $this->match('/<\/svg>.*?Google Playstore.*?<\/th>.*?<td.*?<a href=".*?\?id=(.*?)\.*?".*?<\/a>/ms', $links, 1);	
		
		if ($arr['GP_ID'] === FALSE or $arr['GP_ID'] == '') $arr['GP_ID'] = $arr['GP_IDX'] = $arr['GP_IDX1'];	
		$title_id = $this->match('/<\/svg>.*?Google Playstore.*?<\/th>.*?<td.*?<a href=".*?\?id=(.*?)\.*?".*?<\/a>/ms', $links, 1);		
		...
		$titleId = $arr['GP_ID'];		
		$arr['title'] = str_replace(array('getmodsapk.com', '&#8211;', '-', 'for Android', '+', 'Download'), '', $this->match('/<title>(.*?)<\/title>/ms', $links, 1));
		
		$arr['title2'] = trim(strip_tags($this->match('/<span class="sm:text-left text-center block w-full">(.*?)<\/span>/ms', $links, 1)));
        ...
        $arr['mods'] = str_replace(array('getmodsapk.com', 'MOD ', 'Download'), '', $this->match('/<span class="sm:text-left text-center block w-full">.*?\((.*?)\).*?<\/span>/ms', $links, 1));
		...
		$arr['mods_alt'] = $this->match_all('/<li>(.*?)<\/li>/ms', $this->match('/<div class="faq-description hidden.*?>(.*?)<\/div>/ms', $links, 1), 1);
 
        if ($arr['mods'] === FALSE or $arr['mods'] == array('0', '1', '2')) $arr['mods'] = $arr['mods_alt'][0];
		
		$arr['mods_alt_title'] = trim(strip_tags($this->match('/<div class="flex items-center justify-between">.*?<h3 class="text-lg font-medium font-poppins.*?">(.*?)<\/h3>.*?<span class="ml-4" faq-icon-1>/ms', $links, 1)));
		...
		*/
		$arr['name_downloadlinks_gma'] = $this->match_all('/<span class="closed.*?>(.*?)<\/span>/ms', $this->match('/<div class="pt-8">.*?<ul>(.*?)<\/ul>.*?<\/div>/ms', $download_links_pages, 1), 1);		 
		 
		$arr['size_downloadlinks_gma'] = $this->match_all('/<\/svg>(.*?)<\/a>/ms', $this->match('/<div class="pt-8">.*?<ul>(.*?)<\/ul>.*?<\/div>/ms', $download_links_pages, 1), 1);	
