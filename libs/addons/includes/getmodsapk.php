...
$demos = 'https://getmodsapk.com/grim-soul-dark-fantasy-survival-mod-apk/'; ?>
    <div class="play_menu" style="text-transform:uppercase">
        <b>Add sources from getmodsapk.com</b>
        ...
        <div class="tab-content">
            ...
            <div id="menu2" class="tab-pane fade">
				...
				<b>Latest Uploads Games </b>
                    ...
                    $sumbers = 'https://getmodsapk.com/games'; 
                    $target1 = 'https://getmodsapk.com';
                    $target2 = 'getmodsapk.com';
					$contents =  file_get_contents($sumbers, false, stream_context_create($ssl)); 
					 
					$contents =  preg_replace('#<!DOCTYPE html>.*?relative mt-8">#si', '<div class="relative mt-8">', $contents); 
					$contents =  preg_replace('#<div class="status.*?</div>.*?<div class="post-content block">#si', '<div class="post-content block">', $contents); 
					$contents =  preg_replace('#<div class="label.*?</div>.*?</div>#si', '</div>', $contents); 
					$contents =  preg_replace('#<p class="text-gray-500.*?</p>#si', ' ', $contents); 
					$contents =  preg_replace('#<ul class="flex flex-wrap justify-between.*?</ul>.*?</div> </div>#si', '</div> </div>', $contents); 
					$contents =  preg_replace('#<div>.*?<div class="pagination.*?</html>#si', '', $contents); 
					 
					$contents =  preg_replace('#<div>.*?<div class="pagination.*?</html>#si', '', $contents); 
					$contents =  preg_replace('#<div class="xl:w-2/12 md:w-3/12 w-6/12 sm:p-3 p-1">#si', '', $contents); 
					$contents =  preg_replace('#</div> </div>#si', '</div>', $contents);    
					$contents =  preg_replace('#<div class="post-item bg-white.*?>.*?<div class="post-content block">#si', '<div class="wrapper"><div class="content"><div class="columns">', $contents); 
					$contents =  preg_replace('#<img src=#si', '<img style="float: left;max-width: 75px !important;max-height: 75px!important;border-radius: 10px;margin-bottom: 25px;" class="main" src=', $contents);   
					
					$contents =  preg_replace('#<h3.*?<a href="(.*?)".*?>(.*?)</a>.*?</h3>#si', '<b style="float: left;margin-left: 20px;margin-top: 25px;font-size: 15px;font-weight: bold;margin-bottom: 5px;margin-right: 30px;">$2</b><form name="myForm" id="myForm" method="POST" class="sidebar" style=""  ><input style="display:none" type="text" name="wp_url" value="$1"  /><input type="submit" id="Submit" name="wp_sb" class="button-primary abe2" value="'.postnow2.'"/></form></div>', $contents);
					echo $contents;
					?>   
            ...
            <div id="menu3" class="tab-pane fade">
			   <div class="play_menu" style="text-transform:uppercase">
				<b>Latest Post Apps</b>
                    ...
                    $sumbers = 'https://getmodsapk.com/apps'; 
                    $target1 = 'https://getmodsapk.com';
                    $target2 = 'getmodsapk.com';
					$contents =  file_get_contents($sumbers, false, stream_context_create($ssl)); 
