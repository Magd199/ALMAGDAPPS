    ...
    if(isset($_POST['wp_sb'])) {
        if(isset($_POST['wp_url'])) {
            ...
            if(stristr($url, $sources)) {
                ...
                echo '<div class="play_menu" style=""><h3 style="color:blue;font-size: 15px;">your link is <b style="color:red;text-transform: lowercase;">'.$url.'</b> <u style="color:black;text-transform: lowercase;"> Please check your link..</u> </h3></div>';
            }
        }
    }
    ?>
        ...
        <div class="tab-content">
            ...
            <div id="menu2" class="tab-pane fade">
				...
				<b>Latest Uploads Games </b>
                    ...
                    $sumbers		= 'https://modcombo.com/games'; 
					...
					$contents =  preg_replace('#<!DOCTYPE html>.*?<ul class="apks w3 apkincategories box-space">#si', '<ul class="apks w3 apkincategories box-space">', $contents);    
					
					$contents =  preg_replace('#<div class="pagination box-space">.*?</html>#si', '', $contents);   
					 
					$contents =  str_ireplace('<ul class="apks w3 apkincategories box-space">', '<div class="containerx1">', $contents); 
					$contents =  str_ireplace('</ul>', '</div>', $contents); 
					/*
					<li>.*?<a class="apk-icon" href="(.*?)" title="(.*?)">.*?<figure class="thumb">.*?<img.*?data-src="(.*?)".*?>.*?<\/figure>.*?<\/a>.*?<\/li>
					*/
					$contents =  preg_replace('#<li>.*?<a class="apk-icon" href="(.*?)" title="(.*?)">.*?<figure class="thumb">.*?<img.*?data-src="(.*?)".*?>.*?<\/figure>.*?<\/a>.*?<\/li>#si', '
					<div class="item item--1"><img src="$3" width="75px" height="75px" /></div>
					<div class="item item--2"><b>$2</b></div>
            ...
            <div id="menu3" class="tab-pane fade">
			   <div class="play_menu" style="text-transform:uppercase">
				<b>Latest Post Apps</b>
                    ...
                    $sumbers		= 'https://modcombo.com/apps'; 
					$parse			= parse_url($sumbers);
					$target			= $parse['host'];
                    $target1		= 'https://'.$target;
                    $target2		= $target; 
